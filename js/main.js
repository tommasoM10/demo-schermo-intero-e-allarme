...
async function detectionLoop(){
  ...
  for (const ev of tracker.consumeEvents()){
    saveLog(ev);
    if (ev.type==='prealert'){ hud.style.display='flex'; hud.style.background='#ffa502'; hud.textContent='PRE-ALLERTA: controllo area…'; setTimeout(()=>{ if (hud.style.background!=='#ff4757') hud.style.display='none'; }, 2500); }
...
startBtn.addEventListener('click', async ()=>{
  try{
    if (sourceSelect.value==='camera'){ await startCamera(); } else { startDemo(); hint.style.display='block'; setTimeout(()=>hint.style.display='none', 3000); }
